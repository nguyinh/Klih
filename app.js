...
app.use((req, res, next) => {
  ...
  res.header(
    ...
    "Origin, X-Requested-With, Content-Type, Accept, Cache-Control, X-HTTP-Method-Override"
  );
...
db.on("error", () => {
  logger.error(`[MongoDB] Error during connection with ${mongoURL}`);
});
db.once("open", () => {
...
if (process.env.ENV === "dev") server.listen(port, "0.0.0.0");
